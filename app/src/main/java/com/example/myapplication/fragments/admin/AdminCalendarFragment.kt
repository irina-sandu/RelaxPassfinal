...
import com.example.myapplication.databinding.FragmentAdminCalendarBinding
import com.example.myapplication.fragments.DetailsTicketFragment
...
class AdminCalendarFragment : Fragment(), OnClickListener {
    private lateinit var binding: FragmentAdminCalendarBinding
    ...
    private fun setCalendar() {
        ...
        binding.calendarView.setOnDateChangeListener { view, year, month, dayOfMonth ->
            ...
            if (selectedCalendar.before(calendar)) {
                Toast.makeText(
                    requireContext(),
                    "Nu puteți selecta zilele din trecut",
                    Toast.LENGTH_SHORT
                ).show()
                binding.calendarView.date = calendar.timeInMillis
            } else {
                val selectedDate =
                    "${dayOfMonth}/${month + 1}/${year}" // +1 la month pentru că este indexat de la 0

                val newTicketList = filterTicketsByDate(selectedCalendar)
                Log.d("Calendar List", newTicketList.toString())

                binding.recycleviewTicketItemFragmentCalendarAdmin.layoutManager =
                    LinearLayoutManager(requireContext())
                adapter = TicketAdapter(newTicketList as ArrayList<Ticket>, this)
                binding.recycleviewTicketItemFragmentCalendarAdmin.adapter = adapter
            }
        }
    }
    ...
    fun filterTicketsByDate(date: Calendar): List<Ticket> {
        ...
        for (ticket in ticketList!!) {
            val ticketDate =
                ticket.data.substring(0, 10) // Extrage doar partea de data din String-ul de data
    ...
    override fun onClickListenerDetails(ticketPos: Int) {
        //Nu trebuie sa faca nimic aceasta functie momentan
    }
}